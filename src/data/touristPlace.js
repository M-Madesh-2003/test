import wonderla from "../assets/wonderla/wonderla.avif";
import wonderla2 from "../assets/wonderla/wonderla2.avif";
import wonderla3 from "../assets/wonderla/wonderla3.avif";
import wonderla4 from "../assets/wonderla/wonderla4.avif";
import wonderla5 from "../assets/wonderla/wonderla5.avif";
import wonderlavideo from '../assets/wonderla/wonderlavideo.mp4'

import palace1 from "../assets/mysore/palace1.avif";
import palace2 from "../assets/mysore/palace2.avif";
import palace3 from "../assets/mysore/palace3.avif";
import palace4 from "../assets/mysore/palace4.avif";
import palace5 from "../assets/mysore/palace5.avif";

import coorg from "../assets/coorg/coorg.jpg";
import coorg1 from "../assets/coorg/coorg1.jpg";
import coorg2 from "../assets/coorg/coorg2.jpg";
import coorg3 from "../assets/coorg/coorg3.jpg";
import coorg4 from "../assets/coorg/coorg4.jpg";
import coorg5 from "../assets/coorg/coorg5.jpg";

import nandi from "../assets/nandi/nandi.jpg";
import nandi1 from "../assets/nandi/nandi1.jpg";
import nandi2 from "../assets/nandi/nandi2.jpg";
import nandi3 from "../assets/nandi/nandi3.jpg";

import jollywood from "../assets/jollywood/jollywood.jpg";
import jollywood1 from "../assets/jollywood/jollywood1.jpg";
import jollywood2 from "../assets/jollywood/jollywood2.jpg";
import jollywood3 from "../assets/jollywood/jollywood3.jpg";
import jollywood4 from "../assets/jollywood/jollywood4.jpg";

import kanva from "../assets/kanvalake/kanvalake.avif";
import kanva1 from "../assets/kanvalake/kanvalake1.avif";
import kanva2 from "../assets/kanvalake/kanvalake2.avif";
import kanva3 from "../assets/kanvalake/kanvalake3.avif";
import kanva4 from "../assets/kanvalake/kanvalake4.avif";

import ramanagara from "../assets/ramanagara/ramanagara.avif";
import ramanagara1 from "../assets/ramanagara/ramanagara1.avif";
import ramanagara2 from "../assets/ramanagara/ramanagara2.avif";
import ramanagara3 from "../assets/ramanagara/ramanagara3.avif";

import iskon from "../assets/iskon/iskon.jpg";
import iskon1 from "../assets/iskon/iskon1.jpg";
import iskon2 from "../assets/iskon/iskon2.jpg";
import iskon3 from "../assets/iskon/iskon3.jpg";

import banashankari from "../assets/banashankari/banashankari.webp";
import banashankari1 from "../assets/banashankari/banashankari1.webp";
import banashankari2 from "../assets/banashankari/banashankari2.webp";
import banashankari3 from "../assets/banashankari/banashankari3.webp";

import shivoham from "../assets/Shivoham/Shivoham.jpg";
import shivoham1 from "../assets/Shivoham/Shivoham1.jpg";
import shivoham2 from "../assets/Shivoham/Shivoham2.jpg";
import shivoham3 from "../assets/Shivoham/Shivoham3.jpg";

import lalbagh from "../assets/lalbagh/lalbagh.jpg";
import lalbagh1 from "../assets/lalbagh/lalbagh1.jpg";
import lalbagh2 from "../assets/lalbagh/lalbagh2.jpg";
import lalbagh3 from "../assets/lalbagh/lalbagh3.jpg";
import lalbagh4 from "../assets/lalbagh/lalbagh4.jpg";

import Bannerughatta from "../assets/Bannerughatta/Bannerughatta.jpg";
import Bannerughatta1 from "../assets/Bannerughatta/Bannerughatta1.webp";
import Bannerughatta2 from "../assets/Bannerughatta/Bannerughatta2.webp";
import Bannerughatta3 from "../assets/Bannerughatta/Bannerughatta3.webp";
import Bannerughatta4 from "../assets/Bannerughatta/Bannerughatta4.jpg";

import cubbon from "../assets/cubbon/cubbon.jpg";
import cubbon1 from "../assets/cubbon/cubbon1.jpg";
import cubbon2 from "../assets/cubbon/cubbon2.jpg";
import cubbon3 from "../assets/cubbon/cubbon3.jpg";
import cubbon4 from "../assets/cubbon/cubbon4.jpg";
import cubbon5 from "../assets/cubbon/cubbon5.jpg";

import gatewayofindia from "../assets/gatewayofindia/gatewayofindia.jpg";
import gatewayofindia1 from "../assets/gatewayofindia/gatewayofindia1.jpg";
import gatewayofindia2 from "../assets/gatewayofindia/gatewayofindia2.jpg";
import gatewayofindia3 from "../assets/gatewayofindia/gatewayofindia3.jpg";
import gatewayofindia4 from "../assets/gatewayofindia/gatewayofindia4.jpg";

import tajmahal from "../assets/tajmahal/Tajmahal.jpg";
import tajmahal1 from "../assets/tajmahal/Tajmahal1.jpg";
import tajmahal2 from "../assets/tajmahal/Tajmahal2.jpg";
import tajmahal3 from "../assets/tajmahal/Tajmahal3.jpg";
import tajmahal4 from "../assets/tajmahal/Tajmahal4.jpg";

import eiffeltower from "../assets/eiffeltower/eiffeltower.webp";
import eiffeltower1 from "../assets/eiffeltower/eiffeltower1.webp";
import eiffeltower2 from "../assets/eiffeltower/eiffeltower2.webp";
import eiffeltower3 from "../assets/eiffeltower/eiffeltower3.webp";
import eiffeltower4 from "../assets/eiffeltower/eiffeltower4.webp";

import auqa from '../assets/aquabar/aqua1.jpg'
import auqa1 from '../assets/aquabar/aqua2.jpg'
import auqa2 from '../assets/aquabar/aqua3.jpg'
import auqa3 from '../assets/aquabar/aqua4.jpg'

import nolimits from '../assets/nolimits/nolimits.jpg'
import nolimits1 from '../assets/nolimits/nolimits1.jpg'
import nolimits2 from '../assets/nolimits/nolimits2.jpg'
import nolimits3 from '../assets/nolimits/nolimits3.jpg'

import kittyko from '../assets/kittyko/kittyko1.jpg'
import kittyko1 from '../assets/kittyko/kittyko2.jpg'
import kittyko2 from '../assets/kittyko/kittyko3.jpg'
import kittyko3 from '../assets/kittyko/kittyko4.jpg'

export const places = [
  {
    name: "Wonderla",
    mainImg: wonderla,
    sliderImgs: [wonderla, wonderla2, wonderla3, wonderla4, wonderla5],
    category: "Theme Park",
    about:
      "Wonderla Holidays Limited, is the No. 1 amusement park operator in India. Promoted by Arun K. Chittilappilly and Kochouseph Chittilappilly, Wonderla currently owns and operates three amusement parks – one each in Kochi, Bangalore and Hyderabad respectively and a resort in Bangalore. Wonderla Kochi, spread over 35 acres of land, is thrilling visitors with 56 rides since 2000. Wonderla Bangalore, spanning over 82 acres, offers a spectacular range of entertainment with 61 rides. Wonderla Hyderabad, the latest edition in the portfolio, is spread over 50 acres and offers 43 attractions. Wonderla Amusement Park has been ranked No. 1 in India and No. 6 in Asia by TripAdvisor for 5 consecutive years. Wonderla Resort, located beside the amusement park in Bangalore, comprises of 84 luxury rooms, with amenities including 4 full-fledged banquet halls, multi cuisine restaurant, rest-o-bar and recreational facilities like a heated swimming pool, kid’s activity centre & a full fledged gym.",
    age: "No limit",
    timing: [
      "Monday 11:00 AM - 6:00 PM",
      "Tuesday 11:00 AM - 6:00 PM",
      "Wednesday 11:00 AM - 6:00 PM",
      "Thursday 11:00 AM - 6:00 PM",
      "Friday 11:00 AM - 6:00 PM",
      "Saturday 11:00 AM - 7:00 PM",
      "Sunday 11:00 AM - 7:00 PM",
    ],
    ticketPrice: {
      children: 1212.1,
      adult: 1516.72,
      senior: 1137.08,
    },
    type: ["Adventurous", "Educational", "vecation"],
    vdosrc: wonderlavideo,
    reviews: 886,
    ratings: 4,
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    entryType: "cost",
    cost: 1517,
    link: "https://www.wonderla.com/bengaluru-amusement-park/",
  },
  {
    name: "Gateway Of India",
    mainImg: gatewayofindia,
    sliderImgs: [
      gatewayofindia,
      gatewayofindia1,
      gatewayofindia2,
      gatewayofindia3,
      gatewayofindia4,
    ],
    category: "Heritage Site",
    about:
      "Built in 1911 to welcome the King and Queen of England, this stone archway in Colaba is now a historic landmark.",
    age: "No limit",
    timing: [
      "Monday 24 Hrs",
      "Tuesday 24 Hrs",
      "Wednesday 24 Hrs",
      "Thursday 24 Hrs",
      "Friday 24 Hrs",
      "Saturday 24 Hrs",
      "Sunday 24 Hrs",
    ],
    type: ["Educational", "vecation"],
    reviews: 1232,
    ratings: 3,
    location: {
      city: "Mumbai",
      state: "Maharastra",
      country: "India",
    },
    entryType: "free",
    cost: 0,
    link: "https://www.wonderla.com/bengaluru-amusement-park/",
  },
  {
    name: "Taj Mahal",
    mainImg: tajmahal,
    sliderImgs: [tajmahal, tajmahal1, tajmahal2, tajmahal3, tajmahal4],
    category: "Historical Monument",
    type: ["Educational", "vecation"],
    about:
      "The Taj Mahal, located in Agra, India, is a magnificent white marble mausoleum built by Mughal Emperor Shah Jahan in memory of his beloved wife, Mumtaz Mahal. It is renowned for its intricate architecture, symmetrical design, and exquisite gardens, symbolizing eternal love. Recognized as a UNESCO World Heritage Site, it is one of the Seven Wonders of the World.",
    age: "No limit",
    timing: [
      "Monday 12:00 AM - 11:59 PM",
      "Tuesday 12:00 AM - 11:59 PM",
      "Wednesday 12:00 AM - 11:59 PM",
      "Thursday 12:00 AM - 11:59 PM",
      "Friday 12:00 AM - 11:59 PM",
      "Saturday 12:00 AM - 11:59 PM",
      "Sunday 12:00 AM - 11:59 PM",
    ],
    reviews: 4532,
    ratings: 4,
    ticketPrice: {
      children: 50,
      adult: 50,
      senior: 50,
    },
    location: {
      city: "Agra",
      state: "Uttar Pradesh",
      country: "India",
    },
    entryType: "cost",
    cost: 50,
    link: "https://www.tajmahal.gov.in/",
  },
  {
    name: "Iskon Temple",
    mainImg: iskon,
    sliderImgs: [iskon, iskon1, iskon2, iskon3],
    category: "Devotional",
    type: ["Devotional"],
    about:
      "ISKCON Sri Radha Krishna temple was inaugurated in the year 1997. It is not just a temple, but a cultural complex housing the temples dedicated to the Deities of Sri Sri Radha Krishnachandra, Sri Sri Krishna Balarama, Sri Sri Nitai Gauranga, Sri Srinivasa Govinda, Sri Prahlada Narasimha, Bhakta Hanuman, Garudadeva and Srila Prabhupada, Founder Acharya of ISKCON. ISKCON Bangalore is a charitable society with the objective of propagating Krishna Consciousness all over the world, as explained by Srila Prabhupada, whose teachings are based on Bhagavad-gita and Srimad Bhagavatam.",
    timing: [
      "Monday 4:15 AM - 8:15 AM",
      "Tuesday 4:15 AM - 8:15 AM",
      "Wednesday 4:15 AM - 8:15 AM",
      "Thursday 4:15 AM - 8:15 AM",
      "Friday 4:15 AM - 8:15 AM",
      "Saturday 4:15 AM - 8:15 AM",
      "Sunday 4:15 AM - 8:15 AM",
    ],
    age: "No limit",
    ratings: 2,
    reviews: 73,
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    entryType: "Free",
    cost: 0,
    link: "https://www.iskconbangalore.org/",
  },
  {
    name: "Eiffel Tower",
    mainImg: eiffeltower,
    sliderImgs: [
      eiffeltower,
      eiffeltower1,
      eiffeltower2,
      eiffeltower3,
      eiffeltower4,
    ],
    category: "Cultural Heritage",
    about:
      "Completed in 1889, this colossal landmark, although initially hated by many Parisians, is now a famous symbol of French civic pride.",
    ratings: 4,
    age: "No limit",
    reviews: 3254,
    ticketPrice: {
      adult: 1800,
      children: 900,
      senior: 1500,
    },
    timing: [
      "Monday 8:45 AM - 11:45 PM",
      "Tuesday 8:45 AM - 11:45 PM",
      "Wednesday 8:45 AM - 11:45 PM",
      "Thursday 8:45 AM - 11:45 PM",
      "Friday 8:45 AM - 11:45 PM",
      "Saturday 8:45 AM - 11:45 PM",
      "Sunday 8:45 AM - 11:45 PM",
    ],
    type: ["vecation"],
    location: {
      city: "Paris",
      country: "France",
    },
    entryType: "cost",
    cost: 1800,
    link: "https://www.toureiffel.paris/en",
  },
  {
    name: "Ramanagara Adventure",
    mainImg: ramanagara,
    sliderImgs: [ramanagara, ramanagara1, ramanagara2, ramanagara3],
    category: "Camping",
    type: ["Adventurous", "vecation"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    about:
      "Ramanagara Adventure Camp offers an exciting camping experience amidst the lush greenery and majestic mountain views of Ramanagara. Known for its perfect blend of natural beauty and adventure, the camp provides a range of exhilarating activities, including trekking, cave exploration, ziplining, and swimming in the scenic lake. Visitors can also enjoy fun indoor-outdoor games like table tennis, cricket, and archery.",
    ratings: 3,
    reviews: 1597,
    age: "No limit",
    timing: [
      "Monday 24 Hrs",
      "Tuesday 24 Hrs",
      "Wednesday 24 Hrs",
      "Thursday 24 Hrs",
      "Friday 24 Hrs",
      "Saturday 24 Hrs",
      "Sunday 24 Hrs",
    ],
    ticketPrice: {
      adult: 1059,
      children: 799,
      senior: 999,
    },
    entryType: "cost",
    cost: 1059,
    link: "https://www.thrillophilia.com/tours/adventure-camp-in-ramanagara",
  },
  {
    name: "Cubbon Park",
    mainImg: cubbon,
    sliderImgs: [cubbon, cubbon1, cubbon2, cubbon3, cubbon4, cubbon5],
    category: "Park",
    type: ["Educational", "Leisure"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Cubbon Park dons many hats: a green lung in the heart of the city that also hosts a library, museums, a tennis academy, an aquarium, a toy train and many statues and pavilions. It’s probably one of the only parks to have a busy road cutting through it. In the wee hours of the morning or evenings, it’s a jogger’s paradise. Spring adds to the beauty of this park, with the lovely and colorful Tabebuia trees in full bloom.",
    ratings: 4,
    reviews: 864,
    timing: [
      "Monday 5 AM - 9 PM",
      "Tuesday 5 AM - 9 PM",
      "Wednesday 5 AM - 9 PM",
      "Thursday 5 AM - 9 PM",
      "Friday 5 AM - 9 PM",
      "Saturday 5 AM - 9 PM",
      "Sunday 5 AM - 9 PM",
    ],
    entryType: "Free",
    cost: 0,
    link: "https://karnatakatourism.org/tour-item/cubbon-park/",
  },
  {
    name: "Coorg Adventure Resort",
    mainImg: coorg,
    sliderImgs: [coorg, coorg1, coorg2, coorg3, coorg4, coorg5],
    category: "Resort",
    type: ["Adventurous", "Vacation", "Leisure"],
    location: {
      city: "Coorg",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "From thrilling trekking trails and heart-pounding whitewater rafting to serene nature walks and camping under the starlit sky, Coorg offers a myriad of outdoor activities that cater to adrenaline junkies and nature enthusiasts alike.",
    ratings: 4,
    reviews: 122,
    timing: [
      "Monday 24 Hrs",
      "Tuesday 24 Hrs",
      "Wednesday 24 Hrs",
      "Thursday 24 Hrs",
      "Friday 24 Hrs",
      "Saturday 24 Hrs",
      "Sunday 24 Hrs",
    ],
    ticketPrice: {
      adult: 2500,
      children: 1800,
      senior: 2500,
    },
    entryType: "cost",
    cost: 5000,
    link: "https://coorgadventureresort.com",
  },
  {
    name: "Mysore Palace",
    mainImg: palace1,
    sliderImgs: [palace1, palace2, palace3, palace4, palace5],
    category: "Historical",
    type: ["Educational", "Vacation"],
    location: {
      city: "Mysore",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Mysore Palace, also known as Amba Vilas Palace, is a historical palace and a royal residence. It is located in Mysore, Karnataka, India. It used to be the official residence of the Wadiyar dynasty and the seat of the Kingdom of Mysore. The palace is in the centre of Mysore, and faces the Chamundi Hills eastward. ",
    ratings: 5,
    reviews: "2.8L",
    timing: [
      "Monday 10 AM - 5:30 PM",
      "Tuesday 10 AM - 5:30 PM",
      "Wednesday 10 AM - 5:30 PM",
      "Thursday 10 AM - 5:30 PM",
      "Friday 10 AM - 5:30 PM",
      "Saturday 10 AM - 5:30 PM",
      "Sunday 10 AM - 5:30 PM",
    ],
    ticketPrice: {
      adult: 1000,
      children: 800,
      senior: 1000,
    },
    entryType: "cost",
    cost: 300,
    link: "https://mysorepalace.gov.in",
  },
  {
    name: "Nandi Hills Camping",
    mainImg: nandi,
    sliderImgs: [nandi, nandi1, nandi2, nandi3],
    category: "Camping",
    type: ["Adventurous", "Vacation"],
    location: {
      city: "Chikkaballapur",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Nandi Hills, or Nandidurg, is a hill fortress in the south Indian state of Karnataka. Tipu Sultan Fort, a summer retreat of the namesake 18th-century ruler, features stone carvings and wall paintings. Prisoners are said to have been thrown to their death from Tipu’s Drop, now known for its panoramic views. Local Hindu temples include the hilltop Yoga Nandeeshwara Temple, guarded by a huge statue of a bull (nandi).",
    ratings: 5,
    reviews: 8234,
    timing: [
      "Monday 24 Hrs",
      "Tuesday 24 Hrs",
      "Wednesday 24 Hrs",
      "Thursday 24 Hrs",
      "Friday 24 Hrs",
      "Saturday 24 Hrs",
      "Sunday 24 Hrs",
    ],
    ticketPrice: {
      adult: 1500,
      children: 700,
      senior: 1500,
    },
    entryType: "cost",
    cost: 1500,
    link: "https://nandihillscamping.com",
  },
  {
    name: "Bannerughatta Biological",
    mainImg: Bannerughatta,
    sliderImgs: [
      Bannerughatta,
      Bannerughatta1,
      Bannerughatta2,
      Bannerughatta3,
      Bannerughatta4,
    ],
    about:
      "Vast national reserve featuring a zoo, safari park, butterfly conservatory & animal rescue center.",
    ratings: 5,
    reviews: "3K",
    timing: [
      "Monday 9:30 AM – 5 PM",
      "Tuesday 9:30 AM – 5 PM",
      "Wednesday 9:30 AM – 5 PM",
      "Thursday 9:30 AM – 5 PM",
      "Friday 9:30 AM – 5 PM",
      "Saturday 9:30 AM – 5 PM",
      "Sunday 9:30 AM – 5 PM",
    ],
    ticketPrice: {
      adult: 260,
      children: 150,
      senior: 250,
    },
    age: "No limit",
    category: "Park",
    type: ["Educational", "Leisure"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    entryType: "cost",
    cost: 260,
    link: "https://bannerughattabiopark.org/",
  },
  {
    name: "Jollywood Studios",
    mainImg: jollywood,
    sliderImgs: [jollywood, jollywood1, jollywood2, jollywood3, jollywood4],
    category: "Theme Park",
    type: ["Adventurous", "Educational", "vecation"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Jollywood Studios and Adventures appears to be a fictional or lesser-known theme park or entertainment entity, possibly inspired by Hollywood or Bollywood-style attractions. ",
    ratings: 3,
    reviews: "2.3K",
    timing: [
      "Monday 10:45 AM – 7:30 PM",
      "Tuesday 10:45 AM – 7:30 PM",
      "Wednesday 10:45 AM – 7:30 PM",
      "Thursday 10:45 AM – 7:30 PM",
      "Friday 10:45 AM – 7:30 PM",
      "Saturday 10:45 AM – 7:30 PM",
      "Sunday 10:45 AM – 7:30 PM",
    ],
    ticketPrice: {
      adult: 1179,
      children: 900,
      senior: 1099,
    },
    entryType: "cost",
    cost: 1179,
    link: "https://www.jollywood.co.in/",
  },
  {
    name: "Kanva Lake Camping",
    mainImg: kanva,
    sliderImgs: [kanva, kanva1, kanva2, kanva3, kanva4],
    category: "Camping",
    type: ["Adventurous", "vecation"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Kanva Lake Camping provides a serene getaway surrounded by nature's beauty, making it a must-try camping experience for those seeking relaxation and adventure. Nestled beside the tranquil Kanva Lake, this campsite offers picturesque views and a peaceful atmosphere for all visitors. A range of outdoor activities await you at the camp, including kayaking, canoeing and others, allowing you to fully immerse themselves in the natural surroundings.",
    ratings: 4,
    reviews: 144,
    timing: [
      "Monday 24 Hrs",
      "Tuesday 24 Hrs",
      "Wednesday 24 Hrs",
      "Thursday 24 Hrs",
      "Friday 24 Hrs",
      "Saturday 24 Hrs",
      "Sunday 24 Hrs",
    ],
    ticketPrice: {
      adult: 1349,
      children: 899,
      senior: 1199,
    },
    entryType: "cost",
    cost: 1349,
    link: "https://www.thrillophilia.com/tours/lakeside-camping-in-kanva",
  },
  {
    name: "Shree Banashankari Devi",
    mainImg: banashankari,
    sliderImgs: [banashankari, banashankari1, banashankari2, banashankari3],
    category: "Devotional",
    type: ["Devotional"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Shree Banashankari Devi, also known as Banashankari Amma, is a revered Hindu goddess primarily worshipped in Karnataka, India. She is considered a form of Goddess Parvati, consort of Lord Shiva, and is often associated with Shakti (divine feminine energy).",
    ratings: 5,
    reviews: "22K",
    timing: [
      "Monday 6:00  AM – 9:00 PM",
      "Tuesday 6:00  AM – 9:00 PM",
      "Wednesday 6:00  AM – 9:00 PM",
      "Thursday 6:00  AM – 9:00 PM",
      "Friday 6:00  AM – 9:00 PM",
      "Saturday 6:00  AM – 9:00 PM",
      "Sunday 6:00  AM – 9:00 PM",
    ],
    entryType: "Free",
    cost: 0,
    link: "https://www.avathi.com/place/banashankari-temple/2625",
  },
  {
    name: "Shivoham Shiva",
    mainImg: shivoham,
    sliderImgs: [shivoham, shivoham1, shivoham2, shivoham3],
    category: "Devotional",
    type: ["Devotional"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "On February 26, 1995, on the auspicious day of Maha Shivratri, the Shivoham Shiva Temple was inaugurated by Sri Shankaracharya of Shringeri. Since then the Shiva Temple has become a revered spiritual destination for devotees and visitors. Year after year, with Lord Shiva’s blessings, the temple has transformed into a destination of faith that attracts millions of devotees from not only India but all around the world. The main attractions of the temple include the 65-feet-tall Shiva statue, the 32-feet-tall Ganesha statue, the 25-feet-tall Shiv Ling, Paanch Dham Yatra, and Barah Jyothir Ling Yatra.",
    ratings: 4,
    reviews: "13K",
    timing: [
      "Monday 24 Hrs",
      "Tuesday 24 Hrs",
      "Wednesday 24 Hrs",
      "Thursday 24 Hrs",
      "Friday 24 Hrs",
      "Saturday 24 Hrs",
      "Sunday 24 Hrs",
    ],
    entryType: "Free",
    cost: 0,
    link: "https://www.avathi.com/place/banashankari-temple/2625",
  },
  {
    name: "Lalbagh Botanical Garden",
    mainImg: lalbagh,
    sliderImgs: [lalbagh, lalbagh1, lalbagh2, lalbagh3, lalbagh4],
    category: "Park",
    type: ["Educational", "Leisure"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "No limit",
    about:
      "Lalbagh Botanical Garden or simply Lalbagh, is a botanical garden in Bangalore, India, with an over 200-year history. First planned and laid out during the dalavaiship of King Hyder Ali, the garden was later managed under numerous British Superintendents before Indian Independence.",
    ratings: 4,
    reviews: 4543,
    timing: [
      "Monday 6:00  AM – 7:00 PM",
      "Tuesday 6:00  AM – 7:00 PM",
      "Wednesday 6:00  AM – 7:00 PM",
      "Thursday 6:00  AM – 7:00 PM",
      "Friday 6:00  AM – 7:00 PM",
      "Saturday 6:00  AM – 7:00 PM",
      "Sunday 6:00  AM – 7:00 PM",
    ],
    ticketPrice: {
      adult: 25,
      children: 15,
      senior: 25,
    },
    entryType: "cost",
    cost: 25,
    link: "https://lalbaghbotanicalgarden.in/",
  },
  {
    name: "Aqua",
    mainImg: auqa,
    sliderImgs: [auqa, auqa1, auqa2, auqa3],
    category: "Party",
    type: ["Party"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "Above 21",
    about:
      "Stylish hotel bar serving cocktails & spirits with lounge seating on a chic poolside patio.",
    ratings: 4,
    reviews: 4543,
    timing: [
      "Monday 11:00 AM – 1:00 AM",
      "Tuesday 11:00 AM – 1:00 AM",
      "Wednesday 11:00 AM – 1:00 AM",
      "Thursday 11:00 AM – 1:00 AM",
      "Friday 11:00 AM – 1:00 AM",
      "Saturday 11:00 AM – 1:00 AM",
      "Sunday 11:00 AM – 1:00 AM",
    ],
    ticketPrice: {
      adult: 2000,
    },
    entryType: "cost",
    cost: 2000,
    link: "https://lalbaghbotanicalgarden.in/",
  },
  {
    name: "Nolimmits Lounge & Club",
    mainImg: nolimits,
    sliderImgs: [nolimits, nolimits1, nolimits2, nolimits3],
    category: "Party",
    type: ["Party"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "Above 21",
    about:
      "Happening, neon-lit nightclub & lounge featuring DJ'd music, plus bar food with veggie options.",
    ratings: 3,
    reviews: 4543,
    timing: [
      "Monday 11:00 AM - 1:00 AM",
      "Tuesday 11:00 AM - 1:00 AM",
      "Wednesday 11:00 AM - 1:00 AM",
      "Thursday 11:00 AM - 1:00 AM",
      "Friday 11:00 AM - 1:00 AM",
      "Saturday 11:00 AM - 1:00 AM",
      "Sunday 11:00 AM - 1:00 AM",
    ],
    ticketPrice: {
      adult: 2000,
    },
    entryType: "cost",
    cost: 2000,
    link: "https://lalbaghbotanicalgarden.in/",
  },
  {
    name: "Kitty Ko",
    mainImg: kittyko,
    sliderImgs: [kittyko, kittyko1, kittyko2, kittyko3],
    category: "Party",
    type: ["Party"],
    location: {
      city: "Bengaluru",
      state: "Karnataka",
      country: "India",
    },
    age: "Above 21",
    about:
      "Swanky nightspot at the The LaLiT hotel, offering cocktails & a patio with city vistas.",
    ratings: 4,
    reviews: 2342,
    timing: [
      "Monday 07:00 AM - 11:30 PM",
      "Tuesday 07:00 AM - 11:30 PM",
      "Wednesday 07:00 AM - 11:30 PM",
      "Thursday 07:00 AM - 11:30 PM",
      "Friday 07:00 AM - 11:30 PM",
      "Saturday 07:00 AM - 11:30 PM",
      "Sunday 07:00 AM - 11:30 PM",
    ],
    ticketPrice: {
      adult: 2000,
    },
    entryType: "cost",
    cost: 2000,
    link: "https://lalbaghbotanicalgarden.in/",
  },
];
